...
describe MailingList::Steps::LifeStage do
  ...
  before do
    allow(instance).to receive(:other_step).with(:degree_status) { instance_double(MailingList::Steps::DegreeStatus, degree_status_step_conditions) }
  ...
  context "when does not have a degree" do
    let(:degree_status_step_conditions) { { has_degree?: false, degree_in_progress?: false, no_degree?: true } }
